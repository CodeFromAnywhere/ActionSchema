...
export const findMyTables = async (context) => {
    ...
    const absolutedbFilePaths = (await fs.promises.readdir(absoluteFilesPath, {
        encoding: "utf8",
    }))
        .map((filename) => path.join(absoluteFilesPath, filename, `${filename}.mdb`))
        .filter(fs.existsSync);
    const tables = (await Promise.all(absolutedbFilePaths.map(async (absolutePath) => {
        if (!projectRoot) {
            return;
        }
...
findMyTables.config = { isPublic: true };
//# sourceMappingURL=findMyTables.js.map
import { fs, getFolderSize } from "fs-util";
...
export const calculateProjectSizes = async () => {
    ...
    const existingProjectPaths = filePaths
        ...
        .flat();
    const calculatedProjectSizes = await Promise.all((await Promise.all(existingProjectPaths.map(async (absolutePath) => {
        ...
        const isUpdateNeeded = !!lastOperationAt &&
        ...
        .map(async (db) => {
        // calculate size
        const absoluteFolderPath = path.join(projectRoot, path.parse(db.projectRelativePath).dir);
...
//# sourceMappingURL=calculateProjectSizes.js.map
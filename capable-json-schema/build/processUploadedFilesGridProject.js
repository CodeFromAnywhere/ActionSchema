...
import { isAllTrue, mergeObjectsArray, notEmpty } from "js-util";
...
export const getTableItems = async (firstFileProjectRelativePath) => {
    const absolutePath = path.join(projectRoot, firstFileProjectRelativePath);
    const firstFileExtension = getExtension(firstFileProjectRelativePath).toLowerCase();
    if (firstFileExtension === "csv") {
        // csv to array
        const array = await tryParseCsv(absolutePath);
        const items = array.map((item) => {
            const newItem = mergeObjectsArray(Object.keys(item).map((key) => {
...
export const processUploadedFilesGridProject = async (context) => {
    const { projectRelativeFilePaths, projectRelativePath, ...standardContext } = context;
    const canRead = isAllTrue((await Promise.all(projectRelativeFilePaths.map((p) => {
    ...
    const firstFileExtension = projectRelativeFilePaths[0]
        ...
        : undefined;
    const isGridFile = projectRelativeFilePaths.length === 1 &&
        firstFileExtension &&
    ...
    else if (projectRelativeFilePaths.length === 1 &&
...
//# sourceMappingURL=processUploadedFilesGridProject.js.map
...
export const getOpenapi = async (c) => {
    ...
    const isApi = hostname === "api.codefromanywhere.com" ||
        hostname === "api.codefromanywhere.localhost";
    const isSdk = hostname === "sdk.codefromanywhere.com" ||
    ...
    */
        const fns = isSdk
            ...
            : await tryRequireFunctionsWithConfig(([name, item]) => !!item.config?.plugin);
        if (!fns) {
        ...
        const paths = mergeObjectsArray(modelPaths.map((x) => x.pathsObjectPart).filter(notEmpty));
        const otherDefs = mergeObjectsArray(modelPaths.map((x) => x.definitions).filter(notEmpty));
        const openapi = {
            openapi: "3.1.0",
            info: {

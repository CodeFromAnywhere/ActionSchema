 ...
 */
export const getOpenapiDetails = async ($openapi, actionSchemaPlugins) => {
    if (!$openapi) {
    ...
    const plugin = exactPlugin
        ...
        : // NB: Temporary Exception
            $openapi.url === "https://root.actionschema.com/openapi.json"
                ? actionSchemaPlugins?.find((x) => x.__id?.endsWith(".actionschema.com/openapi.json"))
                : undefined;
    if (!plugin) {
    ...
    const realOpenapiUrl = (isDev || plugin.isInternallyHosted) &&
...
//# sourceMappingURL=getOpenapiDetails.js.map
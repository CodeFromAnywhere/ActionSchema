import { CapableJsonSchema, MainCapableJsonSchema } from ".";
import { camelCase } from "convert-case";
/** goal would be to visualise the life-cycle of an actionschema */
export const getMermaidVisualisationCode = (schema: MainCapableJsonSchema) => {
  ...
  const dependencies = Object.keys(properties)
    .map((key) => {
      const value = properties[key] as CapableJsonSchema;
      ...
      return firstPlugin.propertyDependencies
        .map(
          (dep) =>
            `${camelCase(dep)} --> |use ${firstPlugin.$openapi

...
import { mapMany, mergeObjectsArray, notEmpty, sum } from "js-util";
...
export const executeGridPlugin = async (context) => {
    const { projectRelativePath, rowIds, mode, propertyKey, waitForResult, ...standardContext } = context;
    if (!propertyKey ||
        !rowIds ||
        !mode ||
        !projectRelativePath ||
    ...
    const { $openapi, context: pluginContext, priceCredit, outputLocation, propertyDependencies, isVerticalExpandEnabled, condition, } = firstPlugin;
    ...
    const requiredRows = rows.filter((row) => {
        ...
        return isExecuteRequired(row, propertyKey, mode, propertyDependencies, isVerticalExpandEnabled, condition);
    });
    const requiredRowIds = requiredRows
        .map((x) => x.__actionSchemaId)
        .filter(notEmpty);
    ...
    await db.addDelta(requiredRowIds.map((rowId) => ({
        newStatus: "busy",
        rowId,
        propertyKey,
    })));
    await db.put("status", mergeObjectsArray(requiredRowIds.map((id) => ({
        [id]: { [propertyKey]: "busy" },
    }))));
    const resultPromise = mapMany(requiredRowIds, async (id) => {
        const row = db.getItem(id);
        if (!row) {
            return;
        }
        ...
        const locatedResult = outputLocation && outputLocation !== ""
            ? getKeyFromObject(result, outputLocation || "result")
            : result;
        console.log({ result, locatedResult });
        const doVerticalExpand = isVerticalExpandEnabled && Array.isArray(locatedResult);
    ...
    const promise = resultPromise.then(async (results) => {
        ...
        const totalPriceCredit = sum(priceCreditRows.map((x) => x.priceCredit).filter(notEmpty));

 ...
 *
 */
export const executeGridSourcePlugin = async (context) => {
    const { projectRelativePath, plugin, shouldExecuteGridEntireRow, partitionAmount, ...standardContext } = context;
    const realPartitionAmount = !partitionAmount || isNaN(partitionAmount) || partitionAmount < 1
        ? 1
        : partitionAmount;
    ...
    const { $openapi, partitionContext, partitionContextsDone, priceCredit, propertyDependencies, } = plugin;
    ...
    const partitionableFunctionName = functionItem.config?.partitionableFunctionName;
    ...
    const completeContext = partitionContext
    ...
    const availablePartitionContexts = partitionContexts.filter((item) => {
        const isAlreadyDone = !!(partitionContextsDone || []).find((x) => deepEqual(item, x));
        return !isAlreadyDone;
    });
    if (availablePartitionContexts.length === 0) {
        return {
            isSuccessful: false,
            message: "We have already done all possible values.. Please choose something different.",
        };
    }
    const partitionContextsNow = availablePartitionContexts.slice(0, realPartitionAmount);
    console.log({ realPartitionAmount, length: partitionContextsNow.length });
    const completePartitionContexts = partitionContextsNow.map((item) => ({
        ...item,
        ...standardContext,
    }));
    ...
    oneByOne(completePartitionContexts, async (completePartitionContext, index) => {
        const result = await partitionableFunctionItem.fn(completePartitionContext);
        ...
        const plugins = schema.properties.items.creationPlugins.map((x) => {
            if (x.$openapi?.operationId === plugin.$openapi?.operationId) {
                return {
                    ...x,
                    partitionContextsDone: (x.partitionContextsDone || []).concat(partitionContextsNow[index]),
                };
            }
            return x;
        });
...
//# sourceMappingURL=executeGridSourcePlugin.js.map
...
import {
  executeFunctionWithPaywall,
  getFunctionItemWithName,
} from "function-wrappers";
...
const bunServerPath = path.join(
  projectRoot,
  operations["actionschema-homeserver"],
);
if (process.cwd() !== bunServerPath) {
  console.log(
    "Proces must be launced from actionschema-homeserver path (absolute paths aren't supported in hono/node)",
    bunServerPath,
  );
  process.exit();
}
...
app.get("/", (c) => {
  ...
  return c.html(
    `<html><body><iframe width="100%" height="100%" src="https://preview.readme.io/reference?url=${encodeURIComponent(
      openapiUrl,
    )}" /></body></html>`,
    200,
  );
...
app.post("*", async (c) => {
  ...
  if (pathname.startsWith("/function/")) {
    ...
    const headerObject = {
      "openai-ephemeral-user-id": request.headers.get(
